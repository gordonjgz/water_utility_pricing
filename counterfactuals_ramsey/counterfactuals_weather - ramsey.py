...
def mean_ignore_outliers(arr, lower_percentile=25, upper_percentile=75):
    # Compute the IQR along axis=0
    q1 = jnp.percentile(arr, lower_percentile, axis=0)
    q3 = jnp.percentile(arr, upper_percentile, axis=0)
    iqr = q3 - q1
    ...
    # Compute the sum of the filtered array along axis=0
    return jnp.mean(filtered_arr, axis=0)

# JIT the function
mean_ignore_outliers_jitted = jax.jit(mean_ignore_outliers)
...
def get_v_out(q_sum_hh, p_l, q_l, fc_l, Z):
    ...
    #inverse_mu = jnp.power(sim_result_Ik, rho)
    #result = jnp.multiply(v_out, inverse_mu)
    return v_out
...
#Array(24358482.7035515, dtype=float64)
r_agg_0 = nansum_ignore_nan_inf_jitted(r0 )/12
#Array(7258745.08835472, dtype=float64)
...
def get_e_new_v_p0(q_sum_hh, p_l, q_l, fc_l, Z):
    ...
    A_term = jnp.exp(jnp.dot(A_current_outdoor, b1) + jnp.dot(Z, b2) + c_o+eta_l)
    e_out = jnp.power(jnp.maximum(jnp.multiply((1-rho),
                         (v_out + jnp.multiply(A_term, 
                                    jnp.divide(jnp.power(mp_0, 1-alpha), 1-alpha )))), 1e-16), 1/(1-rho))
    #e_out =jnp.exp(v_out + jnp.multiply(A_term, jnp.divide(jnp.power(mp_0, 1-alpha), 1-alpha ))) 
...
def get_compensating_variation (cs1):
    ...
    Utility is quasi-linear or homothetic in income (which your utility form approximates),
...
def get_v_out_cv0(p_l, q_l, fc_l, I_adj):
    exp_v0 = get_expenditure_in_v_out_jitted(q_sum_hh0, p_l0,q_l0, fc_l0, Z_current)
    v_out = -1 *exp_v0  + jnp.divide(jnp.power(I_adj, (1-rho)), (1-rho))
    return v_out
get_v_out_cv0_jitted = jax.jit(get_v_out_cv0)

"""
from jax.scipy.optimize import root_scalar
@jit
def equation_to_solve(cv, V1, I, d_k0, p0):
    I_adj = I - cv + jnp.multiply(d_k0, de)
    V = get_v_out_cv0_jitted(p_l0, q_l0, fc_l0, I_adj)
    return V - V1

def solve_exact_cv(V1, I, d_k0, p_l0, q_l0, fc_l0, bracket=(0.0, 10_000.0)):
    sol = root_scalar(
        equation_to_solve,
        args=(V1, I, d_k0, p_l0, q_l0, fc_l0),
        method='brentq',
        bracket=bracket
    )
    return sol.root if sol.converged else jnp.nan

def get_exact_compensating_variation (cs1):
    
    #explicitly solve for CV
    
    solve_exact_cv(cs1,I=I, d_k0=d_k0, p_l0=p_l0, q_l0=q_l0, fc_l0=fc_l0)
    return result
get_exact_compensating_variation_jitted = jax.jit(get_exact_compensating_variation)
"""
...
def revenue_compare(r, r0_benchmark =r0_sum_filtered):
    ...
    #result = result.at[jnp.where(mask)].set(r[mask])
...
r0_soft_constraint = 0.8*r0_sum_filtered ### pick 0.8 of status quo revenue to already provide soft constraint, 
### as it's unreasonable to ask each month to be larger than status quo

#r0_benchmark =r0_soft_constraint/12
#r0_benchmark_crra =crra_jitted(r0_soft_constraint/12, gamma)
...
def get_result (p_l, q_l, fc_l, Z):
    ...
    cs_fullyear = nansum_ignore_nan_inf_jitted(cs)
    """   
    total_r = jnp.array([
    ...
    #loss = -1 * lambda_* jnp.sum(loss_function_jitted(crra_total_r - r0_benchmark))

    loss = -1 * lambda_* jnp.sum(loss_function_jitted(total_r - r0_benchmark))
    result =ev_fullyear + loss
    """
    result = cs_fullyear
...
def revenue_lower_bound_constraint(param, Z):
    ...
    """     
    #log_total_r = jnp.log(jnp.maximum(total_r, 1e-16))  
    #result = revenue_compare_jitted(jnp.sum(log_total_r)/12)
    """   
    crra_total_r = crra_jitted(jnp.maximum(total_r, 1e-16), gamma)   
    result = revenue_compare_jitted(jnp.sum(crra_total_r)/12)
    """   
    """
    # Create a mask for valid entries
    max_len = padded_month_indices.shape[1]  # Maximum possible entries in a month
...
constraint2_crra = NonlinearConstraint(revenue_lower_bound_crra_constraint_jitted, 
                                 0.0, jnp.inf, jac='2-point', hess=BFGS())
...
for step in steps:
    try:
        ...
        #cs_step = jnp.multiply((cs_step - cs_0), imu_step)
        cs_step_agg = nansum_ignore_nan_inf_jitted(cs_step) / 12
...
pl_step_agg_results_df.to_csv("ramsey_price_result/price_detail_results/current_info_avg_bound_loss1_mean_pl.csv", index=False)
...
ql_step_agg_results_df.to_csv("ramsey_price_result/price_detail_results/current_info_avg_bound_loss1_mean_ql.csv", index=False)
...
fcl_step_agg_results_df.to_csv("ramsey_price_result/price_detail_results/current_info_avg_bound_loss1_mean_fcl.csv", index=False)
del fcl_step_agg_results, fcl_step_agg_results_df

r_step_agg_results = jnp.array(r_step_agg_results)
r_step_agg_results_df = pd.DataFrame(r_step_agg_results)
r_step_agg_results_df.to_csv("ramsey_welfare_result/current_info_avg_bound_loss1_mean_r.csv", index=False)

cs_step_agg_results = jnp.array(cs_step_agg_results)
cs_step_agg_results_df = pd.DataFrame(cs_step_agg_results)
cs_step_agg_results_df.to_csv("ramsey_welfare_result/current_info_avg_bound_loss1_mean_cs.csv", index=False)

q_step_agg_results = jnp.array(q_step_agg_results)
q_step_agg_results_df = pd.DataFrame(q_step_agg_results)
q_step_agg_results_df.to_csv("ramsey_welfare_result/current_info_avg_bound_loss1_mean_q.csv", index=False)

cs_step_results = jnp.array(cs_step_results)
cs_step_results=cs_step_results.T
cs_step_results_df = pd.DataFrame(cs_step_results)
cs_step_results_df.to_csv("ramsey_welfare_result/cs_detail_results/current_info_avg_bound_loss1_mean_cs_steps.csv", index=False)
del cs_step_results, cs_step_results_df

q_step_results = jnp.array(q_step_results)
q_step_results=q_step_results.T
q_step_results_df = pd.DataFrame(q_step_results)
q_step_results_df.to_csv("ramsey_welfare_result/cs_detail_results/current_info_avg_bound_loss1_mean_q_steps.csv", index=False)
...
r_step_results_df.to_csv("ramsey_welfare_result/cs_detail_results/current_info_avg_bound_loss1_mean_r_steps.csv", index=False)
...
"""
def scale_lognormal_sd(arr, scale_factor, epsilon=1e-9):

    #Scales the standard deviation of log-normally distributed data.

   # This works by transforming the data to the log space (where it is
   # normally distributed), scaling the deviations from the mean in that
    #log space, and then transforming back to the original space via exponentiation.

   # Args:
   #     arr: JAX array containing log-normally distributed data.
  #           Assumed to be positive. Scaling is performed column-wise if arr is 2D.
   #     scale_factor: The factor by which to scale the standard deviation
   #                   in the log space. E.g., 0.5 halves the log-std dev,
   #                   2.0 doubles it.
   #     epsilon: A small value added to 'arr' before taking the logarithm
   #              to prevent log(0) errors and handle near-zero values gracefully.

   # Returns:
   #     A JAX array with the standard deviation scaled appropriately for a
  #      log-normal distribution.

    # Ensure data is positive before log transform by adding a small epsilon
    # or taking the maximum with epsilon. Using maximum is often safer.
    safe_arr = jnp.maximum(arr, epsilon)
    log_arr = jnp.log(safe_arr)

    # Calculate the mean in the log space
    log_mean = jnp.mean(log_arr, axis=0, keepdims=True)

    # Calculate the deviations from the mean in the log space
    log_deviations = log_arr - log_mean

    # Scale the deviations in the log space
    scaled_log_deviations = log_deviations * scale_factor

    # Add the scaled deviations back to the log mean
    rescaled_log = log_mean + scaled_log_deviations

    # Transform back to the original (log-normal) space
    rescaled_arr = jnp.exp(rescaled_log)

    # Optional: Ensure non-negative values if scaling could theoretically
    # produce values near zero that become negative due to numerical issues,
    # although exponentiation should naturally handle this.
    # rescaled_arr = jnp.maximum(rescaled_arr, 0) # Usually not needed after exp

    return rescaled_arr

scale_lognormal_sd_jitted = jax.jit(scale_lognormal_sd)
"""
def scale_lognormal_sd(arr, scale_factor, epsilon=1e-9):
    """
    Scales the standard deviation of a log-normally distributed array
    in the original space by adjusting the standard deviation in log-space,
    while approximately preserving the median (log-space mean).

    Args:
        arr: The input array assumed to be log-normally distributed.
        scale_factor: The factor by which to scale the standard deviation
                      in the original space.
        epsilon: A small value to handle zeros or negative values in the input array.

    Returns:
        A new array with the standard deviation scaled by scale_factor.
    """
    # Ensure all elements are positive for log transformation
    safe_arr = jnp.maximum(arr, epsilon)
    ...
    log_mean = jnp.mean(log_arr, axis=0, keepdims=True)
    log_std = jnp.std(log_arr, axis=0, keepdims=True)

    # Squeeze dimensions for scalar log-space parameters (assuming statistics
    # were calculated over the first axis, resulting in a single set of mu and sigma)
    # If your statistics are calculated over multiple axes or you expect
    # log_mean/log_std to have dimensions that shouldn't be squeezed,
    # you might need to adjust this based on your array structure.
    mu = jnp.squeeze(log_mean)
    sigma = jnp.squeeze(log_std)

    # Compute current standard deviation in original space using the log-normal formula
    # STD = sqrt((exp(sigma^2) - 1) * exp(2*mu + sigma^2))
    original_std = jnp.sqrt((jnp.exp(sigma**2) - 1) * jnp.exp(2 * mu + sigma**2))

    # Compute the desired standard deviation in the original space
    desired_std = original_std * scale_factor

    # --- Solve for the new log-space standard deviation (new_sigma) ---
    # We need to find new_sigma such that the log-normal standard deviation
    # with parameters mu and new_sigma equals desired_std.
    # We use binary search to find the root of the equation:
    # sqrt((exp(sigma'^2) - 1) * exp(2*mu + sigma'^2)) - desired_std = 0

    def find_sigma(current_sigma, target_std, mean_mu):
        # Define the function whose root we seek:
        # f(s) = sqrt((exp(s^2) - 1) * exp(2*mean_mu + s^2)) - target_std
        # We are looking for s such that f(s) = 0.
        # This function is monotonically increasing for s > 0.

        # Binary search range for new_sigma.
        # Increased upper bound and iterations for potentially better convergence.
        lower = jnp.array(1e-6, dtype=arr.dtype) # Sigma must be positive
        upper = jnp.array(30.0, dtype=arr.dtype) # Search up to a larger sigma value

        # Number of binary search iterations. Increased for better accuracy.
        num_iterations = 200 # Increased from 30

        # Perform binary search
        for _ in range(num_iterations):
            mid = (lower + upper) / 2.0
            # Calculate the standard deviation in original space for the current 'mid' sigma
            current_std_for_mid_sigma = jnp.sqrt((jnp.exp(mid**2) - 1.0) * jnp.exp(2.0 * mean_mu + mid**2))

            # Update lower and upper bounds based on comparison with target_std
            # If current_std_for_mid_sigma is greater than target_std, the target sigma
            # is in the lower half of the current interval [lower, mid].
            upper = jnp.where(current_std_for_mid_sigma > target_std, mid, upper)
            # If current_std_for_mid_sigma is less than target_std, the target sigma
            # is in the upper half of the current interval [mid, upper].
            lower = jnp.where(current_std_for_mid_sigma < target_std, mid, lower)

            # Handle the case where current_std_for_mid_sigma is very close to target_std
            # (optional, as binary search naturally converges)
            # equal_mask = jnp.isclose(current_std_for_mid_sigma, target_std, atol=1e-5)
            # lower = jnp.where(equal_mask, mid, lower)
            # upper = jnp.where(equal_mask, mid, upper)


        # The approximate solution for new_sigma is the midpoint of the final interval
        new_sigma_solution = (lower + upper) / 2.0

        # Add a safeguard for potential issues where new_sigma ends up being zero or negative
        # due to numerical instability or target_std being unachievable in the range.
        # Although the lower bound starts at 1e-6, this adds robustness.
        return jnp.maximum(new_sigma_solution, epsilon)


    # Find the new log-space standard deviation
    new_sigma = find_sigma(sigma, desired_std, mu)

    # Compute the scaling factor for log-space deviations
    # Avoid division by zero if original sigma was zero (perfectly uniform in log space)
    # In such cases, scaling is not well-defined in this framework.
    # We'll add a small epsilon to sigma in the denominator.
    log_scale = new_sigma / (sigma + epsilon)

    # Apply scaling to the deviations from the mean in log space
    log_deviation = log_arr - log_mean
    scaled_log = log_mean + log_deviation * log_scale

    # Transform back to original space
    return jnp.exp(scaled_log)
...
"""
"""
param0_high = jnp.array([3, 5, 5, 5, 5, 
                    2, 
                    6-2, 11-6, 20-11,
                    8.5, 
                    7.125, 7.125, 7.125
...
param0_med = jnp.array([3, 4, 4, 4, 4, 
                    2, 
                    6-2, 11-6, 20-11,
                    8.5, 
                   # 0, 0, 0, 0
...
def get_initial_param_for_step(step, param_high, param_med, param_low):
    """
    ...
    if abs(step - 0.75) < tolerance or (step >= 1.1 - tolerance and step <= 1.25 + tolerance):
        ...
        return param_med
    elif step >= 0.8 - tolerance and step <= 1.05 + tolerance:
...
# Export to CSV
np.savetxt("ramsey_welfare_result/cs_detail_results/cs_r_q_avg_115.csv", cs_r_q, delimiter=",", fmt="%.5f")
"""        
...
pl_step_agg_results_iqr_df.to_csv("ramsey_price_result/price_detail_results/current_info_avg_bound_loss1_var_pl.csv", index=False)
...
ql_step_agg_results_iqr_df.to_csv("ramsey_price_result/price_detail_results/current_info_avg_bound_loss1_var_ql.csv", index=False)
...
fcl_step_agg_results_iqr_df.to_csv("ramsey_price_result/price_detail_results/current_info_avg_bound_loss1_var_fcl.csv", index=False)
del fcl_step_agg_results_iqr, fcl_step_agg_results_iqr_df

r_step_agg_results_iqr = jnp.array(r_step_agg_results_iqr)
r_step_agg_results_iqr_df = pd.DataFrame(r_step_agg_results_iqr)
r_step_agg_results_iqr_df.to_csv("ramsey_welfare_result/current_info_avg_bound_loss1_var_r.csv", index=False)

cs_step_agg_results_iqr = jnp.array(cs_step_agg_results_iqr)
cs_step_agg_results_iqr_df = pd.DataFrame(cs_step_agg_results_iqr)
cs_step_agg_results_iqr_df.to_csv("ramsey_welfare_result/current_info_avg_bound_loss1_var_cs.csv", index=False)

q_step_agg_results_iqr = jnp.array(q_step_agg_results_iqr)
q_step_agg_results_iqr_df = pd.DataFrame(q_step_agg_results_iqr)
q_step_agg_results_iqr_df.to_csv("ramsey_welfare_result/current_info_avg_bound_loss1_var_q.csv", index=False)

cs_step_results_iqr = jnp.array(cs_step_results_iqr)
cs_step_results_iqr=cs_step_results_iqr.T
cs_step_results_iqr_df = pd.DataFrame(cs_step_results_iqr)
cs_step_results_iqr_df.to_csv("ramsey_welfare_result/cs_detail_results/current_info_avg_bound_loss1_var_cs_steps.csv", index=False)
del cs_step_results_iqr, cs_step_results_iqr_df
...
q_step_results_iqr_df.to_csv("ramsey_welfare_result/cs_detail_results/current_info_avg_bound_loss1_var_q_steps.csv", index=False)
...
r_step_results_iqr_df.to_csv("ramsey_welfare_result/cs_detail_results/current_info_avg_bound_loss1_var_r_steps.csv", index=False)
...
#results_df['mean_cs_I'] = jnp.divide(results_df['mean_cs'], I)
#results_df['var_cs_I'] = jnp.divide(results_df['var_cs'], I)

results_df.to_csv("ramsey_welfare_result/current_info_avg_bound_loss1_result.csv", index=False)

######################################################################
############## Run for CRRA Gamma = 1 #####################
######################################################################
...
for step in steps:
    try:
        ...
        # First optimization attempt
        solution1_nobd = cobyqa.minimize(
            lambda x: objective0_crra(x, Z_step), 
            param0_high_np,
            bounds=bounds0, 
            constraints=(constraint_conserve
                         , constraint_revenue
...
pl_step_agg_results  = pl_step_agg_results.T
pl_step_agg_results_df = pd.DataFrame(pl_step_agg_results)
pl_step_agg_results_df.to_csv("ramsey_price_result/price_detail_results/current_info_logr_bound_loss1_mean_pl.csv", index=False)
del pl_step_agg_results, pl_step_agg_results_df

ql_step_agg_results = jnp.array(ql_step_agg_results)
ql_step_agg_results  = ql_step_agg_results.T
ql_step_agg_results_df = pd.DataFrame(ql_step_agg_results)
ql_step_agg_results_df.to_csv("ramsey_price_result/price_detail_results/current_info_logr_bound_loss1_mean_ql.csv", index=False)
del ql_step_agg_results, ql_step_agg_results_df

fcl_step_agg_results = jnp.array(fcl_step_agg_results)
fcl_step_agg_results  = fcl_step_agg_results.T
fcl_step_agg_results_df = pd.DataFrame(fcl_step_agg_results)
fcl_step_agg_results_df.to_csv("ramsey_price_result/price_detail_results/current_info_logr_bound_loss1_mean_fcl.csv", index=False)
del fcl_step_agg_results, fcl_step_agg_results_df

r_step_agg_results = jnp.array(r_step_agg_results)
r_step_agg_results_df = pd.DataFrame(r_step_agg_results)
r_step_agg_results_df.to_csv("ramsey_welfare_result/current_info_logr_bound_loss1_mean_r.csv", index=False)

cs_step_agg_results = jnp.array(cs_step_agg_results)
cs_step_agg_results_df = pd.DataFrame(cs_step_agg_results)
cs_step_agg_results_df.to_csv("ramsey_welfare_result/current_info_logr_bound_loss1_mean_cs.csv", index=False)

q_step_agg_results = jnp.array(q_step_agg_results)
q_step_agg_results_df = pd.DataFrame(q_step_agg_results)
q_step_agg_results_df.to_csv("ramsey_welfare_result/current_info_logr_bound_loss1_mean_q.csv", index=False)

cs_step_results = jnp.array(cs_step_results)
cs_step_results=cs_step_results.T
cs_step_results_df = pd.DataFrame(cs_step_results)
cs_step_results_df.to_csv("ramsey_welfare_result/cs_detail_results/current_info_logr_bound_loss1_mean_cs_steps.csv", index=False)
del cs_step_results, cs_step_results_df

q_step_results = jnp.array(q_step_results)
q_step_results=q_step_results.T
q_step_results_df = pd.DataFrame(q_step_results)
q_step_results_df.to_csv("ramsey_welfare_result/cs_detail_results/current_info_logr_bound_loss1_mean_q_steps.csv", index=False)
del q_step_results, q_step_results_df

r_step_results = jnp.array(r_step_results)
r_step_results=r_step_results.T
r_step_results_df = pd.DataFrame(r_step_results)
r_step_results_df.to_csv("ramsey_welfare_result/cs_detail_results/current_info_logr_bound_loss1_mean_r_steps.csv", index=False)
del r_step_results, r_step_results_df
...
pl_step_agg_results_iqr  = pl_step_agg_results_iqr.T
pl_step_agg_results_iqr_df = pd.DataFrame(pl_step_agg_results_iqr)
pl_step_agg_results_iqr_df.to_csv("ramsey_price_result/price_detail_results/current_info_logr_bound_loss1_var_pl.csv", index=False)
del pl_step_agg_results_iqr, pl_step_agg_results_iqr_df

ql_step_agg_results_iqr = jnp.array(ql_step_agg_results_iqr)
ql_step_agg_results_iqr  = ql_step_agg_results_iqr.T
ql_step_agg_results_iqr_df = pd.DataFrame(ql_step_agg_results_iqr)
ql_step_agg_results_iqr_df.to_csv("ramsey_price_result/price_detail_results/current_info_logr_bound_loss1_var_ql.csv", index=False)
del ql_step_agg_results_iqr, ql_step_agg_results_iqr_df

fcl_step_agg_results_iqr = jnp.array(fcl_step_agg_results_iqr)
fcl_step_agg_results_iqr  = fcl_step_agg_results_iqr.T
fcl_step_agg_results_iqr_df = pd.DataFrame(fcl_step_agg_results_iqr)
fcl_step_agg_results_iqr_df.to_csv("ramsey_price_result/price_detail_results/current_info_logr_bound_loss1_var_fcl.csv", index=False)
del fcl_step_agg_results_iqr, fcl_step_agg_results_iqr_df

r_step_agg_results_iqr = jnp.array(r_step_agg_results_iqr)
r_step_agg_results_iqr_df = pd.DataFrame(r_step_agg_results_iqr)
r_step_agg_results_iqr_df.to_csv("ramsey_welfare_result/current_info_logr_bound_loss1_var_r.csv", index=False)

cs_step_agg_results_iqr = jnp.array(cs_step_agg_results_iqr)
cs_step_agg_results_iqr_df = pd.DataFrame(cs_step_agg_results_iqr)
cs_step_agg_results_iqr_df.to_csv("ramsey_welfare_result/current_info_logr_bound_loss1_var_cs.csv", index=False)

q_step_agg_results_iqr = jnp.array(q_step_agg_results_iqr)
q_step_agg_results_iqr_df = pd.DataFrame(q_step_agg_results_iqr)
q_step_agg_results_iqr_df.to_csv("ramsey_welfare_result/current_info_logr_bound_loss1_var_q.csv", index=False)

cs_step_results_iqr = jnp.array(cs_step_results_iqr)
cs_step_results_iqr=cs_step_results_iqr.T
cs_step_results_iqr_df = pd.DataFrame(cs_step_results_iqr)
cs_step_results_iqr_df.to_csv("ramsey_welfare_result/cs_detail_results/current_info_logr_bound_loss1_var_cs_steps.csv", index=False)
del cs_step_results_iqr, cs_step_results_iqr_df

q_step_results_iqr = jnp.array(q_step_results_iqr)
q_step_results_iqr=q_step_results_iqr.T
q_step_results_iqr_df = pd.DataFrame(q_step_results_iqr)
q_step_results_iqr_df.to_csv("ramsey_welfare_result/cs_detail_results/current_info_logr_bound_loss1_var_q_steps.csv", index=False)
del q_step_results_iqr, q_step_results_iqr_df

r_step_results_iqr = jnp.array(r_step_results_iqr)
r_step_results_iqr=r_step_results_iqr.T
r_step_results_iqr_df = pd.DataFrame(r_step_results_iqr)
r_step_results_iqr_df.to_csv("ramsey_welfare_result/cs_detail_results/current_info_logr_bound_loss1_var_r_steps.csv", index=False)
del r_step_results_iqr, r_step_results_iqr_df
...
results_df.to_csv("ramsey_welfare_result/current_info_logr_bound_loss1_result.csv", index=False)
