...
solution = cobyqa.minimize(objective, param0, bounds = bounds, constraints=(constraint1, constraint2, constraint3), options={'disp': True,
...
#####################################################################################
####### Test The Actual Weather Risk if employing these price structures ############
#####################################################################################
...
###### Full Information
###### ###### ###### ###### ###### ###### ######
param_full = jnp.array([3.3888, 3.5645, 3.4701, 3.1761, 0.39428,
                    2.187119,
                    4.728701, 6.063726, 9.567338,
                    6.602197,
                    0.40519, 3.5066, 19.07251
                    , 0.02696
                    ])

p_lfull, q_lfull, fc_lfull = param_to_pq_jitted(param_full)

def get_q_sum_hh_Z(p_l, q_l, fc_l, Z_indoor, Z_outdoor):
    log_q = cf_w_jitted(p_l, q_l, fc_l, Z_indoor, Z_outdoor)
    chunk_size = 100
    num_columns = log_q.shape[1]
    result_hh = []
    #result_sim = []
    ...
    #q_sum = nansum_ignore_nan_inf_jitted(q)
    return q_sum_hh
get_q_sum_hh_Z_jitted = jax.jit(get_q_sum_hh_Z)

def sum_ignore_outliers(arr, lower_percentile=25, upper_percentile=75):
    # Compute the IQR
    q1 = jnp.percentile(arr, lower_percentile)
    q3 = jnp.percentile(arr, upper_percentile)
    iqr = q3 - q1
    ...
    # Compute the sum of the filtered array
    return jnp.sum(filtered_arr)

Z_outdoor_actual = jnp.column_stack((jnp.array(demand_2018_using_new['mean_TMAX_1']),
                                      jnp.array(demand_2018_using_new['IQR_TMAX_1']),
                                      jnp.array(demand_2018_using_new['total_PRCP'])
                                      ,jnp.array(demand_2018_using_new['IQR_PRCP'])))
Z_indoor_actual = jnp.array(demand_2018_using_new['mean_TMAX_1'])
Z_indoor_actual = Z_indoor_actual[:, jnp.newaxis]


q_sum_hh_full_actual = get_q_sum_hh_Z_jitted(p_lfull, q_lfull, fc_lfull, Z_indoor_actual, Z_outdoor_actual)


Z_outdoor_drought = jnp.column_stack((jnp.array(demand_2018_using_new['mean_Tmax_history']),
                                      jnp.array(demand_2018_using_new['IQR_Tmax_history']),
                                      jnp.array(demand_2018_using_new['sum_Prcp_extreme_min'])
                                      ,jnp.array(demand_2018_using_new['IQR_Prcp_history'])))
Z_indoor_drought = jnp.array(demand_2018_using_new['mean_Tmax_history'])
Z_indoor_drought = Z_indoor_drought[:, jnp.newaxis]

Z_outdoor_flood = jnp.column_stack((jnp.array(demand_2018_using_new['mean_Tmax_history']),
                                      jnp.array(demand_2018_using_new['IQR_Tmax_history']),
                                      jnp.array(demand_2018_using_new['sum_Prcp_extreme_max'])
                                      ,jnp.array(demand_2018_using_new['IQR_Prcp_history'])))
Z_indoor_flood = jnp.array(demand_2018_using_new['mean_Tmax_history'])
Z_indoor_flood = Z_indoor_flood[:, jnp.newaxis]



q_sum_hh_full_drought = get_q_sum_hh_Z_jitted(p_lfull, q_lfull, fc_lfull, Z_indoor_drought, Z_outdoor_drought)

q_sum_hh_full_flood = get_q_sum_hh_Z_jitted(p_lfull, q_lfull, fc_lfull, Z_indoor_flood, Z_outdoor_flood)

full_df = pd.DataFrame(jnp.column_stack((jnp.array(demand_2018_using_new['prem_id']), jnp.array(demand_2018_using_new['bill_ym']),
                                         q_sum_hh_full_actual/sim, q_sum_hh_full_drought/sim, q_sum_hh_full_flood/sim) ), columns=['prem_id', 'bill_ym', 'q_actual', 'q_drought', 'q_flood'])

full_df = full_df[full_df['bill_ym'] >= 201901]

full_df['r_actual'] = get_r_mean_jitted(jnp.array(full_df['q_actual']), p_lfull, q_lfull, fc_lfull)
full_df['r_drought'] = get_r_mean_jitted(jnp.array(full_df['q_drought']), p_lfull, q_lfull, fc_lfull)
full_df['r_flood'] = get_r_mean_jitted(jnp.array(full_df['q_flood']), p_lfull, q_lfull, fc_lfull)



r_drought_full_perct = (sum_ignore_outliers(jnp.array(full_df['r_drought'])) - sum_ignore_outliers(jnp.array(full_df['r_actual'])))/sum_ignore_outliers(jnp.array(full_df['r_actual']))
#### drought - actual
#0.1350252
# $14.58128619 millon usd per year

q_drought_full_perct = (sum_ignore_outliers(jnp.array(full_df['q_drought'])) - sum_ignore_outliers(jnp.array(full_df['q_actual'])))/sum_ignore_outliers(jnp.array(full_df['q_actual']))
#### drought - actual
#0.10071224
# $1.12765302 billon Gallon per year

r_flood_full_perct = (sum_ignore_outliers(jnp.array(full_df['r_flood'])) - sum_ignore_outliers(jnp.array(full_df['r_actual'])))/sum_ignore_outliers(jnp.array(full_df['r_actual']))
#### flood - actual
#-0.1092193
# -11.79452301 millon used per year $

q_flood_full_perct = (sum_ignore_outliers(jnp.array(full_df['q_flood'])) - sum_ignore_outliers(jnp.array(full_df['q_actual'])))/sum_ignore_outliers(jnp.array(full_df['q_actual']))
#### flood - actual
#-0.0779608
#-0.87291007 billon Gallon

def rainfall_loop_full (rain_step):
    Z_outdoor = jnp.column_stack((jnp.array(demand_2018_using_new['mean_Tmax_history']),
                                          jnp.array(demand_2018_using_new['IQR_Tmax_history']),
                                          jnp.maximum(jnp.array(demand_2018_using_new['sum_Prcp_history']) + rain_step, 0)
                                          , jnp.array(demand_2018_using_new['IQR_Prcp_history'])))
    Z_indoor = jnp.array(demand_2018_using_new['mean_Tmax_history'])
    Z_indoor = Z_indoor[:, jnp.newaxis]
    q_hh_full = get_q_sum_hh_Z_jitted(p_lfull, q_lfull, fc_lfull, Z_indoor, Z_outdoor)/sim
    #full_df = pd.DataFrame(
     #   jnp.column_stack((jnp.array(demand_2018_using_new['prem_id']), jnp.array(demand_2018_using_new['bill_ym']),
      #                    q_sum_hh_full_actual / sim, q_sum_hh_full / sim)),
       # columns=['prem_id', 'bill_ym', 'q_actual', 'q'])

    #full_df = full_df[full_df['bill_ym'] >= 201901]
    #full_df['r_actual'] = get_r_mean_jitted(jnp.array(full_df['q_actual']), p_lfull, q_lfull, fc_lfull)
    #full_df['r'] = get_r_mean_jitted(jnp.array(full_df['q']), p_lfull, q_lfull, fc_lfull)
    #r_diff = (sum_ignore_outliers(jnp.array(full_df['r'])) - sum_ignore_outliers(jnp.array(full_df['r_actual'])))/1000000
    #q_diff = (sum_ignore_outliers(jnp.array(full_df['r'])) - sum_ignore_outliers(
        #jnp.array(full_df['r_actual']))) / 1000000
    #r_perct = (sum_ignore_outliers(jnp.array(full_df['r'])) - sum_ignore_outliers(jnp.array(full_df['r_actual'])))/sum_ignore_outliers(jnp.array(full_df['r_actual']))
    #q_perct = (sum_ignore_outliers(jnp.array(full_df['q'])) - sum_ignore_outliers(jnp.array(full_df['q_actual'])))/sum_ignore_outliers(jnp.array(full_df['q_actual']))
    return q_hh_full
            #, q_diff, r_perct, q_perct
rainfall_loop_full_jitted = jax.jit(rainfall_loop_full)

rain_step_seq = jnp.arange(-3, 6, 0.1)

rainfall_loop_full_vectorized = jax.vmap(rainfall_loop_full_jitted)
full_info_rainloop = rainfall_loop_full_vectorized(rain_step_seq)

###### ###### ###### ###### ###### ###### ######
###### No Information
###### ###### ###### ###### ###### ######

param_no = jnp.array([3.05E+00, 3.94E+00, 4.25E+00, 3.24E+00, 1.47E+00,
                    2.73E+00,
                    4.92E+00, 6.70E+00, 1.01E+01,
                    8.88E+00,
                    2.58E+00, 4.90E+00, 2.01E+01
                    , 1.00E-02
                    ])

p_lno, q_lno, fc_lno = param_to_pq_jitted(param_no)

q_sum_hh_no_actual = get_q_sum_hh_Z_jitted(p_lno, q_lno, fc_lno, Z_indoor_actual, Z_outdoor_actual)

q_sum_hh_no_drought = get_q_sum_hh_Z_jitted(p_lno, q_lno, fc_lno, Z_indoor_drought, Z_outdoor_drought)

q_sum_hh_no_flood = get_q_sum_hh_Z_jitted(p_lno, q_lno, fc_lno, Z_indoor_flood, Z_outdoor_flood)

no_df = pd.DataFrame(jnp.column_stack((jnp.array(demand_2018_using_new['prem_id']), jnp.array(demand_2018_using_new['bill_ym']),
                                         q_sum_hh_no_actual/sim, q_sum_hh_no_drought/sim, q_sum_hh_no_flood/sim) ), columns=['prem_id', 'bill_ym', 'q_actual', 'q_drought', 'q_flood'])

no_df = no_df[no_df['bill_ym'] >= 201901]

no_df['r_actual'] = get_r_mean_jitted(jnp.array(no_df['q_actual']), p_lno, q_lno, fc_lno)
no_df['r_drought'] = get_r_mean_jitted(jnp.array(no_df['q_drought']), p_lno, q_lno, fc_lno)
no_df['r_flood'] = get_r_mean_jitted(jnp.array(no_df['q_flood']), p_lno, q_lno, fc_lno)

r_drought_no_perct = (sum_ignore_outliers(jnp.array(no_df['r_drought'])) - sum_ignore_outliers(jnp.array(no_df['r_actual'])))/sum_ignore_outliers(jnp.array(no_df['r_actual']))
#### drought - actual
#0.14542903
# $15.78309469 millon usd per year

q_drought_no_perct = (sum_ignore_outliers(jnp.array(no_df['q_drought'])) - sum_ignore_outliers(jnp.array(no_df['q_actual'])))/sum_ignore_outliers(jnp.array(no_df['q_actual']))
#### drought - actual
#0.10131358
# 1.13541595 billon Gallon per year

r_flood_no_perct = (sum_ignore_outliers(jnp.array(no_df['r_flood'])) - sum_ignore_outliers(jnp.array(no_df['r_actual'])))/sum_ignore_outliers(jnp.array(no_df['r_actual']))
#### flood - actual
#-0.11047015
# -11.98908466 millon used per year $

q_flood_no_perct = (sum_ignore_outliers(jnp.array(no_df['q_flood'])) - sum_ignore_outliers(jnp.array(no_df['q_actual'])))/sum_ignore_outliers(jnp.array(no_df['q_actual']))
#### flood - actual
#-0.07861739
#-0.88106097 billon Gallon


### Comparing No info and Full Info

r_drought_risk_full = jnp.square((jnp.array(full_df['r_drought']) - jnp.array(full_df['r_actual'])))
r_flood_risk_full = jnp.square((jnp.array(full_df['r_flood']) - jnp.array(full_df['r_actual'])))
